...
if (!empty($getdata)) {
	...
	if ($timestamp) {
		...
		file_put_contents("error.log", date("d/m/Y H:i:s", time()) . ": " . "\n        moowoodle error:Some one tried to login to (course id) " . $course_id . "> and user id:" . $user_id . "> & (Moodle) & " . $wp_user_id . ">(WordPress) with  time diffarence of:" . $diff . "min or more. on " . json_encode($timevalue) . " now :" . json_encode(date_create("now")) . "\n", FILE_APPEND);
